...
router.get('/', function(req, res) {
    ...
    var sqlstr = 'SELECT * FROM loopmanage';
    
    if(search !== null || search !== undefined || search !== '')
    {
      sqlstr = sqlstr + " WHERE lname LIKE '%"+search+"%'";
    }

    sqlstr = sqlstr + ' LIMIT '+offset+','+limit;
    var countstr = 'SELECT count(*) AS count FROM loopmanage';
    if(search !== null || search !== undefined || search !== '')
    {
      countstr = countstr + " WHERE lname LIKE '%"+search+"%'";
    }
